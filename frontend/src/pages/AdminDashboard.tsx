...
import type { Raffle, RaffleDetail, Winner } from "../api/types";
...
export default function AdminDashboard() {
  ...
  // Live-refresh the selected raffle's counts while the page is open, so the
  // entry/ticket totals (and the draw button's enabled state) stay current as
  // registrations come in. The entries themselves live on their own page now.
  useEffect(() => {
    if (!selectedId) return;
    const id = setInterval(() => {
      // Don't disturb an in-progress draw or the countdown reveal.
      if (drawInFlight.current || countdown !== null) return;
      getRaffle(selectedId).then(setDetail).catch(() => {});
    }, 8000);
  ...
  async function selectRaffle(id: string) {
    ...
    setError(null);
    try {
      const [d, w] = await Promise.all([getRaffle(id), listWinners(id)]);
      setDetail(d);
      setWinners(w);
    } catch (err) {
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-white p-4 shadow">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => navigate(`/raffles/${detail.id}/entries`)}
                    className="rounded-lg border border-gray-300 px-3 py-1.5 text-sm font-semibold text-gray-700 hover:bg-gray-50"
                  >
                    Entries
                  </button>
